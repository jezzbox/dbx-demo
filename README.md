...
```bash
conda create -n dbx_demo python=3.9
conda activate dbx_demo
```
...
```
This command will create your personal repository under `/Repos/<username>/dbx_demo`.
...
- To trigger the release pipeline, get the current version from the `dbx_demo/__init__.py` file and tag the current code version:
```
